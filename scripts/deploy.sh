...
SSH_HOST="ssh-alexander-leuenberger.nona.company"
...
KESTRA_URL=https://kestra-alexander-leuenberger.nona.company
EOF
...
CB_SERVER_URL=https://cloudbeaver-alexander-leuenberger.nona.company
...
REDIRECT_URL=https://hoppscotch-alexander-leuenberger.nona.company
WHITELISTED_ORIGINS=https://hoppscotch-alexander-leuenberger.nona.company
VITE_BASE_URL=https://hoppscotch-alexander-leuenberger.nona.company
VITE_SHORTCODE_BASE_URL=https://hoppscotch-alexander-leuenberger.nona.company
VITE_ADMIN_URL=https://hoppscotch-alexander-leuenberger.nona.company/admin
VITE_BACKEND_GQL_URL=https://hoppscotch-alexander-leuenberger.nona.company/backend/graphql
VITE_BACKEND_WS_URL=wss://hoppscotch-alexander-leuenberger.nona.company/backend/graphql
VITE_BACKEND_API_URL=https://hoppscotch-alexander-leuenberger.nona.company/backend/v1
...
PREFECT_UI_API_URL=https://prefect-alexander-leuenberger.nona.company/api
EOF
...
LAKEFS_GATEWAYS_S3_DOMAIN_NAME=s3.lakefs-alexander-leuenberger.nona.company
...
NC_PUBLIC_URL=https://nocodb-alexander-leuenberger.nona.company
                    ...
                    SECRETS_PAYLOAD=$(cat <<SECRETS_EOF
{
  ...
  "secrets": [
    ...
    {"secretKey": "GITEA_REPO_URL", "secretValue": "https://git-alexander-leuenberger.nona.company/${ADMIN_USERNAME}/nexus-${DOMAIN//./-}-gitea.git", "tagIds": ["$GITEA_TAG"]},
    ...
    cron: \"*/15 * * * *\"'" >/dev/null 2>&1 || true
            ...
            if echo "$ENABLED_SERVICES" | grep -qw "woodpecker" && [ -n "$WOODPECKER_AGENT_SECRET" ]; then
                ...
                OAUTH_RESULT=$(ssh nexus "curl -s -X POST 'http://localhost:3200/api/v1/user/applications/oauth2' \
                    ...
                    -d '{
                        \"name\": \"Woodpecker CI\",
                        \"redirect_uris\": [\"https://woodpecker-alexander-leuenberger.nona.company/authorize\"],
...
if echo "$ENABLED_SERVICES" | grep -qw "wikijs" && [ -n "$WIKIJS_ADMIN_PASS" ]; then
    (
        ...
        SETUP_PAYLOAD=$(jq -n \
            ...
            --arg url "https://wiki-alexander-leuenberger.nona.company" \
